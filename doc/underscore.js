...
//Current version.
   ...
   /**
    *  The cornerstone, an each implementation, aka forEach. Handles objects with the built-in forEach, arrays, and raw objects.
    *  Delegates to ECMAScript 5's native forEach if available.
    *
    *  @param {Array/Object} obj      the array or the object to loop through
    *  @param {Function}     iterator the function to execute for each element or property of the array / object
    *  @param {Object}       context  the object upon which to invoke the function
    *
    *  the iterator callback accept parameter as 'value_of_current_key', 'current_key', 'object'
    *  @example
    *       each([1, 2, 3, 4], function (value, key, array) {....});
    */
    var each = _.each = _.forEach = function(obj, iterator, context) {
        if (obj == null) return;

        /* delegates to 'native' forEach */
        if (nativeForEach && obj.forEach === nativeForEach) {
            obj.forEach(iterator, context);
        } else if (obj.length === +obj.length) { /* this is an array */
     ...
     *  @return {Object}      results results of applying iterator
     */
...
//The right-associative version of reduce, also known as foldr. Delegates to ECMAScript 5's native reduceRight if available.
    ...
    /**
     *  Determine if AT LEAST ONE element in the object matches a truth test, meaning that the callback return true value
...
     *          return (element >= 10);
     *      }
     *
     *      var passed = [2, 5, 8, 1, 4].any(isBigEnough);  // passed is false
     */
    ...
    /**
     *  Determine if the array or object contains a GIVEN VALUE (not key) (using ===). Aliased as include.
     */
     ...
     *      var zips = _.pluck( addresses, "zip" );  // zips = [ "37212", "37406", "37377", "30078", "80437" ]
     */
...
//Generate an integer Array containing an arithmetic progression. A port of the native Python range() function. See the Python documentation.
     ...
     *  @param {Object}    context object to be binded to
     *  @return {Function}
     *
     *  @example
     *      var func = function(greeting){ return greeting + ': ' + this.name };
     *      func = _.bind(func, {name : 'moe'}, 'hi'); // bind the function to object { name : 'moe'} and pass 'hi' as argument to the func
     *      func();
     *      => 'hi: moe'
     */
...
//Return a sorted list of the function names available on the object. Aliased as methods
     ...
     *  optional param - passed-in object
     */
